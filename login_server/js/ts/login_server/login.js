...
class Login {
    ...
    getPlayerByName() {
        return new Promise((res, rej) => {
            db_1.default.query("SELECT * FROM player WHERE name = ?", this.name, (err, row) => {
                if (err) {
                    this.output = { type: rpc_type_1.default.Error, body: "ERROR! Can't get player", name: this.name };
                    return rej(err);
                }
                if (row[0]) {
    ...
    createPlayerByName() {
        return new Promise((res, rej) => {
            db_1.default.query("INSERT INTO `player` (`name`) VALUES (?)", this.name, (err, row) => {
                if (err) {
                    this.output = { type: rpc_type_1.default.Error, body: "ERROR! Can't create player", name: this.name };
                    return rej(err);
                }
                this.output = { type: rpc_type_1.default.Login, body: "Welcome!", success: true, name: this.name };
    ...
    syncPlayer(player) {
        const output = { type: rpc_type_1.default.Sync, body: JSON.stringify(player), name: "3000", target: "player" };
